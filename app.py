import streamlit as st
import pandas as pd
import folium
from streamlit_folium import st_folium
from folium.plugins import MarkerCluster

st.set_page_config(page_title="Red Nacional TVS - Postventa", layout="wide")

st.markdown("## 🛠️ Red Nacional de Servicios Técnicos")

@st.cache_data
def load_data():
    return pd.read_csv('data_red_nacional_final.csv')

try:
    df = load_data()

    # --- BARRA LATERAL: FILTROS ---
    st.sidebar.header("🔍 Filtros de Búsqueda")
    
    # 1. Filtro por Ciudad Base
    ciudades_lista = sorted(df['CIUDAD BASE'].unique())
    seleccion_ciudad = st.sidebar.multiselect("📍 Seleccionar Ciudad Base:", ciudades_lista)

    # 2. Filtro por Cobertura (Búsqueda de texto)
    # Esto permite buscar cualquier ciudad o provincia mencionada en la columna de cobertura
    busqueda_cobertura = st.sidebar.text_input("❄️ Buscar por zona de Cobertura (ej: Daule, Pastaza):")

    # Aplicar Filtros
    df_display = df.copy()
    
    if seleccion_ciudad:
        df_display = df_display[df_display['CIUDAD BASE'].isin(seleccion_ciudad)]
    
    if busqueda_cobertura:
        # Filtramos filas donde la columna de cobertura contenga el texto ingresado (sin importar mayúsculas)
        df_display = df_display[df_display['COBERTURA INST AA Y LINEA BLANCA'].str.contains(busqueda_cobertura, case=False, na=False)]

    # Lógica de Zoom dinámico
    if not df_display.empty:
        centro_lat = df_display['LAT_VIZ'].mean()
        centro_lon = df_display['LON_VIZ'].mean()
        zoom_level = 12 if (seleccion_ciudad or busqueda_cobertura) else 7
    else:
        centro_lat, centro_lon, zoom_level = -1.45, -78.5, 7
        st.sidebar.warning("⚠️ No se encontraron talleres con esos criterios.")

    st.info(f"Talleres encontrados: **{len(df_display)}**")

    # --- MAPA INTERACTIVO ---
    m = folium.Map(location=[centro_lat, centro_lon], zoom_start=zoom_level, tiles="CartoDB positron")
    marker_cluster = MarkerCluster(options={'maxClusterRadius': 30}).add_to(m)

    for _, row in df_display.iterrows():
        popup_html = f"""
        <div style="font-family: sans-serif; font-size: 12px; width: 220px;">
            <b style="color: #E74C3C; font-size: 14px;">{row['NOMBRE DEL TALLER']}</b><br>
            <hr style="margin: 5px 0;">
            <b>🏠 Dirección:</b> {row['DIRECCION']}<br>
            <b>📞 Contacto:</b> {row['NUMEROS DE CONTACTO']}<br>
            <hr style="margin: 5px 0;">
            <b style="color: #2E86C1;">📍 Cobertura:</b><br>{row['COBERTURA INST AA Y LINEA BLANCA']}
        </div>
        """
        folium.Marker(
            location=[row['LAT_VIZ'], row['LON_VIZ']],
            popup=folium.Popup(popup_html, max_width=250),
            tooltip=f"{row['NOMBRE DEL TALLER']}",
            icon=folium.Icon(color="red", icon="wrench", prefix="fa")
        ).add_to(marker_cluster)

    st_folium(m, width="100%", height=550, key="mapa_cobertura")

    # --- TABLA DE INFORMACIÓN BÁSICA ---
    st.markdown("### 📋 Directorio de Contactos y Coberturas")
    
    # Añadimos la columna de cobertura a la tabla para que sea visible abajo
    cols_tabla = ['NOMBRE DEL TALLER', 'CIUDAD BASE', 'DIRECCION', 'NUMEROS DE CONTACTO', 'COBERTURA INST AA Y LINEA BLANCA']
    
    st.dataframe(
        df_display[cols_tabla], 
        use_container_width=True, 
        hide_index=True
    )

except Exception as e:
    st.error(f"Error en la aplicación: {e}")
